...
class DDPG():
    def __init__(self, state_dim, action_dim, args, device, max_action=1.0,  policy_noise=0.2, noise_clip=0.2):
    ...
    def update_parameters(self, replay_buffer, batch_size=16):
        ...
        with torch.no_grad():
            ...
            target_Q1 = self.critic_target(next_state.to(torch.float), next_action.to(torch.float))
            # target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + not_done * self.discount * target_Q1

        # Get current Q estimates
        current_Q1 = self.critic(state.to(torch.float), action.to(torch.float))
