...
if __name__=="__main__":
    ...
    # --------------------------------------Hyperparameter--------------------------------------------------------------------
    parser.add_argument("--policy_name", type=str, default='DDPG', help="")
    ...
    # Set random seed
    set_seed(8)
    args.wave_length = 3e8 / args.fc  # waveform length
    ...
    for eps in range(int(args.num_episodes)):
        ...
        for t in range(int(args.num_step)):
            ...
            if done:
                ...
                np.save(f"./Learning Curves/TD3/{file_name}_episode_{episode_num + 1}_{args.policy_name}", instant_rewards)
    ...
    # show_nodes(max_index, args.Ny)
    np.savez(f"./Learning Curves/{args.policy_name}/{file_name}_episode_{episode_num + 1}_{args.policy_name}.npz",return_list = instant_rewards, max_index = max_index, 
             max_reward = max_reward, w_opt = w_opt,max_eps_list=max_eps_list)
