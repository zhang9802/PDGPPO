...
class ResNet(torch.nn.Module):
    def __init__(self, Ns, J, width_p, depth_p, Pbs):
        super(ResNet, self).__init__()
        self.Pbs = Pbs
        self.linearIn = nn.Linear(Ns * 2 * J, width_p) # input layer
        self.batchNorm = nn.BatchNorm1d(Ns * 2)
...
h_t = np.zeros((Ns, J), dtype=complex)
for jj in range(J):
    h_t[:,jj] =  np.sqrt(large_loss) * two_dim_steering(dy, dz, theta_target[jj], phi_target[jj], Ny, Nz, lamda).reshape(Ns,)


model = ResNet(Ns, J, width_p, depth_p, Pbs).to("cpu")
...
for ii in range(len(theta_all)):
    for jj in range(len(phi_all)):
        ...
        P_GPPO[ii,jj] = np.linalg.norm(steering_temp.conj().T @ w_GPPO,'fro')**2

        steering_temp = E_PPO @ two_dim_steering(dy, dz, theta_temp, phi_temp, Ny, Nz, lamda)
        P_PPO[ii,jj] = np.linalg.norm(steering_temp.conj().T @ w_PPO,'fro')**2

        steering_temp = E_DQN @ two_dim_steering(dy, dz, theta_temp, phi_temp, Ny, Nz, lamda)
        P_DQN[ii,jj] = np.linalg.norm(steering_temp.conj().T @ w_DQN,'fro')**2

        steering_temp = E_TD3 @ two_dim_steering(dy, dz, theta_temp, phi_temp, Ny, Nz, lamda)
        P_TD3[ii,jj] = np.linalg.norm(steering_temp.conj().T @ w_TD3,'fro')**2

        steering_temp = E_DDPG @ two_dim_steering(dy, dz, theta_temp, phi_temp, Ny, Nz, lamda)
        P_DDPG[ii,jj] = np.linalg.norm(steering_temp.conj().T @ w_DDPG,'fro')**2
...
plt.title('PD-GPPO')
plt.tight_layout()
plt.savefig('./figs/beampattern_1_v1.eps', dpi=600, bbox_inches='tight')
...
plt.title('PD-PPO')
plt.tight_layout()
plt.savefig('./figs/beampattern_2_v1.eps', dpi=600, bbox_inches='tight')
...
plt.title('TD3')
plt.tight_layout()
plt.savefig('./figs/beampattern_3_v1.eps', dpi=600, bbox_inches='tight')
...
plt.title('DDPG')
plt.tight_layout()
plt.savefig('./figs/beampattern_4_v1.eps', dpi=600, bbox_inches='tight')
...
plt.title('PD-DQN')
plt.tight_layout()
plt.savefig('./figs/beampattern_5_v1.eps', dpi=600, bbox_inches='tight')

# mesh = axes[1].pcolormesh(Y, Z,  P_PPO, cmap='viridis', shading='auto')  # 使用pcolormesh
# cbar = fig.colorbar(mesh, ax=axes[1])
# # cbar.set_label('Gain')  # 设置 colorbar 标签
# # contour = ax2.contourf(Y, Z,  P_dB, levels=20, cmap='viridis')
# # fig.colorbar(contour, ax=ax2, shrink=0.8, aspect=5)
# axes[1].scatter(phi_target, theta_target, s=10, c='white')
# axes[1].set_ylabel('Azimuth angle')
# axes[1].set_xlabel('Elevation angle')
# axes[1].set_title('PD-PPO')

# mesh = axes[2].pcolormesh(Y, Z,  P_TD3, cmap='viridis', shading='auto')  # 使用pcolormesh
# cbar = fig.colorbar(mesh, ax=axes[2])
# # cbar.set_label('Gain')  # 设置 colorbar 标签
# # contour = ax2.contourf(Y, Z,  P_dB, levels=20, cmap='viridis')
# # fig.colorbar(contour, ax=ax2, shrink=0.8, aspect=5)
# axes[2].scatter(phi_target, theta_target, s=10, c='white')
# axes[2].set_ylabel('Azimuth angle')
# axes[2].set_xlabel('Elevation angle')
# axes[2].set_title('TD3')


# mesh = axes[3].pcolormesh(Y, Z,  P_DDPG, cmap='viridis', shading='auto')  # 使用pcolormesh
# cbar = fig.colorbar(mesh, ax=axes[3])
# # cbar.set_label('Gain')  # 设置 colorbar 标签
# # contour = ax2.contourf(Y, Z,  P_dB, levels=20, cmap='viridis')
# # fig.colorbar(contour, ax=ax2, shrink=0.8, aspect=5)
# axes[3].scatter(phi_target, theta_target, s=10, c='white')
# axes[3].set_ylabel('Azimuth angle')
# axes[3].set_xlabel('Elevation angle')
# axes[3].set_title('DDPG')


# mesh = axes[4].pcolormesh(Y, Z,  P_DQN, cmap='viridis', shading='auto')  # 使用pcolormesh
# cbar = fig.colorbar(mesh, ax=axes[4])
# # cbar.set_label('Gain')  # 设置 colorbar 标签
# # contour = ax2.contourf(Y, Z,  P_dB, levels=20, cmap='viridis')
# # fig.colorbar(contour, ax=ax2, shrink=0.8, aspect=5)
# axes[4].scatter(phi_target, theta_target, s=10, c='white')
# axes[4].set_ylabel('Azimuth angle')
# axes[4].set_xlabel('Elevation angle')
# axes[4].set_title('PD-DQN')


# plt.tight_layout()
# plt.savefig('beampattern_compare.eps', dpi=600, bbox_inches='tight')