...
if __name__=="__main__":
    ...
    TD3 = np.load('../benchmark/TD3/Learning Curves/TD3/20_1e-05_5e-05_episode_2_TD3.npz')
    mv_TD3 = rl_utils.moving_average(TD3['return_list'], 9)
    DDPG = np.load('../benchmark/TD3/Learning Curves/DDPG/20_1e-05_5e-05_episode_2_DDPG.npz')
    ...
    plt.figure()
    plt.plot( GANPP0['return_list'],color='darkturquoise', alpha=0.3)
    plt.plot(mv_GANPP0,color='darkturquoise',label='PD-GPPO')
    plt.plot( PP0['return_list'],color='mediumblue', alpha=0.3)
    plt.plot(mv_PP0,color='mediumblue',label='PPO')
    plt.plot( TD3['return_list'],color='darkviolet', alpha=0.3)
    plt.plot(mv_TD3,color='darkviolet',label='TD3')
    plt.plot( DDPG['return_list'],'aqua', alpha=0.3)
    plt.plot(mv_DDPG,'aqua',label='DDPG')
    plt.plot( DQN['return_list'],'mediumspringgreen', alpha=0.3)
    ...
    # plt.tight_layout()
    plt.legend()
    plt.savefig(name1 + '.pdf', dpi=1200)
    plt.savefig(name1 + '.tif', dpi=1200)

    name2 = 'compare_pos.tif'
    ...
    plt.savefig(name2, dpi=1200)

    name3 = 'compare_val.tif'
    ...
    ax.set_ylabel('Maximum reward', fontsize=16, labelpad=10)
