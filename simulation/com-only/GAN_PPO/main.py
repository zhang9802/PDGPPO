...
if __name__=="__main__":
    ...
    parser.add_argument("--agent_name", type=str, default='PPO', help="GANPPO, PPO")
    ...
    env = environment_v0.COM(args, Js)
    env_name = 'com_only'
    ...
    plt.figure()
    plt.plot(episodes_list, max_eps_list,'b', label='reward')
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title(args.agent_name + ', max_reward=' + str(max_reward))
    plt.grid()
    plt.legend()
    plt.savefig('./figs/' + args.agent_name + '_eps_reward' + '.png', dpi=900)
