...
plt.plot(ns,DDPG,color='cyan',linestyle='-', marker='d',label='DDPG')
plt.plot(ns,DQN,'palegreen',linestyle='-.', marker='*',label='DQN')
plt.xlabel('Number of activated ports')
plt.ylabel('Sum of achievable rate (bps/Hz)')
plt.legend(loc='lower right')
plt.grid()
plt.xlim([3,6])
plt.ylim([37,46])
plt.savefig('./figs/compare_com_reward_vs_ns.eps',dpi=900)
print(DDPG)
