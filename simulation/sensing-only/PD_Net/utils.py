...
def generate_sensing_dataset(lamda, Wz, Wy, Nz, Ny, n_samples, Ns, ns, large_loss, combinations, J):
    ...
    for iter in range(n_samples):
        ...
        for jj in range(J):
            Hk[iter,:,jj] = np.sqrt(large_loss) * (E @ two_dim_steering(dy, dz, theta[jj], phi[jj], Ny, Nz, lamda)).reshape(Ns,) #np.sqrt(large_loss), todo
...
def plot_2D_beampattern(J, Wy, Wz, theta_target, phi_target, Ny, Nz, lamda, w, E):
    ...
    for j in range(J):
        ...
        P_target.append(np.linalg.norm(steering_target.conj().T @ w,'fro'))
    ...
    for ii in range(len(theta_all)):
        for jj in range(len(phi_all)):
            ...
            P_all[ii,jj] = np.linalg.norm(steering_temp.conj().T @ w,'fro')

    P_dB = 10 * np.log10(P_all/np.max(np.max(P_all)))
    P_target_dB = 10 * np.log10(P_target/np.max(np.max(P_all)))
    # P_dB = P_all
    fig = plt.figure()
    ...
    ax.scatter(phi_target, theta_target, P_target_dB, color='blue', s=100, depthshade=False,zorder=10)
    ...
    fig = plt.figure()
    ax2 = plt.axes()
    mesh = plt.pcolormesh(Y, Z,  P_dB, vmin=-10, vmax=0, cmap='viridis', shading='auto')  # 使用pcolormesh
