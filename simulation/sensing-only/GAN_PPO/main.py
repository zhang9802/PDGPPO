...
class PPO:
    ...
    def __init__(self, state_dim,  action_dim, args, device):
        ...
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=5*args.critic_lr, weight_decay=args.decay)
...
class GANPPO:
    ...
    def __init__(self, state_dim,  action_dim, args, device):
        ...
        self.discriminator_optimizer = torch.optim.Adam(self.discriminator.parameters(), lr=args.critic_lr) #2*args.critic_lr
        self.gamma = args.gamma
        self.lmbda = args.lmbda
    ...
    def update(self, transition_dict):
        ...
        noises = torch.ones_like(rewards).data.normal_(0, 0.5).to(device)  # /15,todo
...
if __name__=="__main__":
    ...
    parser.add_argument("--agent_name", type=str, default='GANPPO', help="PPO, PPO")
    ...
    parser.add_argument("--gamma", type=float, default=0.8, help="discount factor,0.8")
    parser.add_argument("--lmbda", type=float, default=0.8, help=",0.8")
    ...
    parser.add_argument("--decay", default=1e-5, type=float, metavar='G',help='Decay rate for the networks (default: 5e-6)')
    ...
    env = environment_v0.Sensing(args)
    ...
    return_list, max_action, max_reward, max_eps_list = rl_utils.train_on_policy_agent(env, agent, args.num_episodes, args.iter_num)
    ...
    # plt.grid()
    # plt.show()
    name = args.agent_name + '_la_' + str(args.actor_lr) + '_lc_' + str(args.critic_lr) + '_hid_' + str(
        args.hidden_dim) + '_eps_' + str(args.eps) + '_gamma_' + str(args.gamma)
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title(args.agent_name + ', max_reward=' + str(max_reward))
    plt.grid()
    plt.legend()
    plt.savefig('./figs/' + name + '.png', dpi=900)
    np.savez('./results/' + name + '.npz', return_list = return_list, max_index = max_index, max_reward = max_reward, max_eps_list=max_eps_list)
    
    
    print(name, max_reward)
