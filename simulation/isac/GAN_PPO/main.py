...
class GANPPO:
    ...
    def update(self, transition_dict):
        ...
        noises = torch.ones_like(rewards).data.normal_(0, 1).to(device)  # /15,todo
...
if __name__=="__main__":
    ...
    parser.add_argument("--critic_lr", type=float, default=8e-5, help="learning rate of actor newtork")
    parser.add_argument("--hidden_dim", type=int, default=128, help="dimesion of hidden layers")
    parser.add_argument("--gamma", type=float, default=0.8, help="discount factor， 0.95")
    ...
    return_list, max_action, max_reward = rl_utils.train_on_policy_agent(env, agent, args.num_episodes, args.iter_num)
    ...
    # plt.grid()
    # plt.show()
    name = './figs/' + args.agent_name + '_la_' + str(args.actor_lr) + '_lc_' + str(args.critic_lr) + '_hid_' + str(
    ...
    show_nodes(max_index, args.Ny, './figs/pos_' + name + '.png')