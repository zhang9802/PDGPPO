...
class ResNet(torch.nn.Module):
    def __init__(self, args):
        ...
        # self.params = params
        self.linearIn = nn.Linear(args.Ns * (args.K + args.J) * 2, args.width_p) # input layer
...
class ResNet_1(torch.nn.Module):
    def __init__(self, args):
        ...
        # self.params = params
        self.linearIn = nn.Linear(args.Ns * (args.K + args.J) * 2, args.width_d)
        self.batchNorm = nn.BatchNorm1d(args.Ns * args.K * 2)
        self.linear = nn.ModuleList()
        for _ in range(args.depth_d):
            self.linear.append(nn.Linear(args.width_d, args.width_d))

        self.linearOut = nn.Linear(args.width_d, args.K + args.J)
...
def cal_obj(H, x, args, device):
    ...
    :return:
    """
    H_real, H_imag = H[:,:args.Ns * (args.K + args.J)].reshape(args.batch_size, args.K + args.J, args.Ns), H[:,args.Ns * (args.K + args.J):].reshape(args.batch_size, args.K + args.J, args.Ns)
    Rk = torch.zeros((args.batch_size, args.K), dtype=torch.float32).to(device)
    Pj = torch.zeros((args.batch_size, args.J), dtype=torch.float32).to(device)
    for i in range(args.batch_size):
        ...
        for jj in range(args.J):
            H_temp = torch.cat((H_real[i,jj + args.K,:].reshape(-1,1).unsqueeze(0), H_imag[i,jj+ args.K,:].reshape(-1,1).unsqueeze(0)), dim=0)
            x_temp = torch.cat((x[i,:args.Ns].reshape(-1,1).unsqueeze(0), x[i, args.Ns:].reshape(-1,1).unsqueeze(0)), dim=0)
            Pj[i, jj] = torch.linalg.norm(cmul(conjT(H_temp), x_temp),'fro')
...
def train_net(model_p, device, args, optimizer_p,  model_d, optimizer_d, H):
    ...
    for i_epoch in range(args.n_epoch):
        ...
        for step in range(steps_per_epoch):
            ...
            for iter in range(args.iter_num):
                ...
                Rk, Pj = cal_obj(H_shuffeld_batch, x, args, device)
                loss1 = torch.mean(args.penalty_F / Rk)

                g_c = args.Gam_c - Rk
                g_s = args.Gam_s - Pj
                g = torch.cat([g_c, g_s], dim=1)
                loss2 = torch.mean(lambda_1 * g) + args.penalty_c/2 * torch.mean(torch.where(g_c > 0, g_c, 0)**2) + args.penalty_s/2 * torch.mean(torch.where(g_s > 0, g_s, 0)**2)
                loss_p = loss1 + loss2
                loss_d = - loss_p
                if iter%4 > 2:
                    loss_d.backward()
    ...
    name = 'my_network_' + str(args.lr_p) + '_' + str(args.batch_size) + '_' + str(args.width_p) + '.pth'
...
if __name__=="__main__":
    ...
    parser.add_argument("--penalty_c", type=float, default=1e4, help="penalty factor for the communication-related constraints")
    parser.add_argument("--penalty_s", type=float, default=1e7, help="penalty factor for the sensing-related constraints")
    ...
    parser.add_argument("--Gam_c", default=0.1, type=float, metavar='N', help='communication and sensing threshold (default: 0.1 bps)') #check
    parser.add_argument("--Gam_s", default=1e-3, type=float, metavar='N', help='communication and sensing threshold (default: 0.1 bps)')  # check
    ...
    train_data = generate_isac_dataset(lamda, args.Wz, args.Wy, args.Nz, args.Ny, args.n_samples, args.K, args.J, args.Ns, args.ns, args.large_loss,Js)
    ...
    model_p.load_state_dict(torch.load('./weights/my_network_0.0005_128_128.pth', weights_only=True))
    model_p.eval()
    # test phase
    for ii in range(10):
        test_data = generate_isac_dataset(lamda, args.Wz, args.Wy, args.Nz, args.Ny, args.batch_size, args.K, args.J, args.Ns, args.ns, args.large_loss,Js)
        test_data = torch.from_numpy(test_data).float().to(device)
        x = model_p(test_data)
        model_p.zero_grad()
        model_d.zero_grad()

        Rk, Pj = cal_obj(test_data, x, args, device)
        print(np.min(Rk.detach().cpu().numpy()), np.min(Pj.detach().cpu().numpy()))
